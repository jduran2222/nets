<?php	
	...
	$campos="alumnos.idAlumno, alumnos.nombre, alumnos.apellidos, alumnos.curso, notas.idAsignatura, TR10, TR01, TR20, TR02, TR30, TR03, TR40";
        ...
        $sWhere="((alumnos.idAlumno = alumnos_grupo.idAlumno) and (alumnos.idAlumno = notas.idAlumno) and (alumnos_grupo.idGrupo LIKE '%".$query."%')and $ordinario) and (alumnos.curso = '$idCurso')";
        $sWhere.="OR((alumnos.idAlumno = alumnos_grupo.idAlumno) and (alumnos.idAlumno = notas.idAlumno) and (alumnos_grupo.idGrupo LIKE '%".$query."%')and $extraordinario)and (alumnos.curso = '$idCurso')";
	...
	$count_query   = mysqli_query($con,"SELECT distinct count((alumnos.idAlumno)) as numrows from alumnos where idAlumno in (select alumnos.idAlumno FROM $tables where $sWhere GROUP BY alumnos.idAlumno HAVING (COUNT(DISTINCT idAsignatura) = '$asignaturas'))");
	...
	$query1 = mysqli_query($con,"SELECT alumnos.idAlumno, alumnos.nombre, alumnos.apellidos, alumnos.curso, alumnos_grupo.idGrupo FROM  $tables where $sWhere GROUP BY alumnos.idAlumno HAVING COUNT(DISTINCT idAsignatura) = '$asignaturas'");	
